...
class PrivateDocLoader():
    ...
    def drop_collection(self):
        ...
        print(f"Dropping Collection {self.collection_name} SELF {qdrant_client.get_collections().collections}")
        coll_names = list(map(lambda x: x.name, qdrant_client.get_collections().collections))
    ...
    def upload(self, documents) -> None:
        qdrant_doc = Qdrant.from_documents(
            documents, PrivateDocLoader.embeddings, 
