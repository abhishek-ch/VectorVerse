...
from vectordbs import qdrant, chromadb, elasticsearch
...
def explore_ui(vectordbs:list[str]):
    st.session_state['vectorexplore'] = st.sidebar.multiselect("Select VectorDB(s)", vectordbs, key="multiselect")
    ...
    vector_ask = st.sidebar.button('Ask') #,disabled=st.session_state.vector_btn_disabled
    ...
    # if len(st.session_state['vectorexplore']) > 0:
    #     st.session_state["vector_btn_disabled"] = False
    # else:
...
def get_merged_result(documents:list[Document]) -> str:
    ...
    for doc_tuple in documents:
        ...
        st.caption(f"""File: _{metadata.get("file_name")}_ | Chunk Size: _{metadata.get("chunk_size")}_ | Overlap: _{metadata.get("chunk_overlap")}_""")
    ...
    for idx in range(start, len(vectordbs_selected), 2):
        ...
        with column:
            ...
            for idx in range(len(result)):
                ...
                tabs[idx].caption(f"""File Name: _{metadata.get("file_name")}_ | Chunk Size: _{metadata.get("chunk_size")}_ | Overlap: _{metadata.get("chunk_overlap")}_""")
        ...
        with st.container():
            with st.expander(vector_db):
                for doc_tuple in result:
                    ...
                    metadata = document.metadata
                    st.markdown(page_content)
                    st.caption(f"""File: _{metadata.get("file_name")}_ | Chunk Size: _{metadata.get("chunk_size")}_ | Overlap: _{metadata.get("chunk_overlap")}_ | Score: _{score}_""")
