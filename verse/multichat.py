...
def multichat_ui(conn:Connection, vectordbs:list[str]) -> None:
    st.session_state['chatvectordb'] = st.sidebar.selectbox("Select Vector Database", vectordbs, key="selectvectordb")
    st.sidebar.markdown("---\n")
    with st.sidebar.expander("Model Configuration"):
        exec_choice = st.radio("Chat with ? Model", ('All', 'Specific'), horizontal=True)

        if exec_choice == 'Specific':
            st.session_state['singlemodel'] = st.selectbox("Select Model", models, key="singleselect")
        else:
            st.session_state['models'] = st.multiselect("Select Model(s)", models, key="multiselect")
...
def chat_ui(conn:Connection, models: List[str], single_model:str) -> list[DeltaGenerator]:
    if single_model:
    ...
    if ref_models:
        ...
        for idx, tab in enumerate(tab_list):
            with tab:
                with st.empty():
                    update_chat(model=ref_models[idx], records = fetch_history(conn=conn, model=ref_models[idx]))
        return tab_list
    return None
...
#     with tab.container():
#         tab.markdown(result['result'].replace('\n\n', '\n'), unsafe_allow_html=True)
#         tab.markdown("---\n")

def update_chat_window(conn:Connection, query:str, curr_model:str, llm:BaseLLM, retriever:BaseRetriever):
    ...
    insert_chathistory(conn=conn, model=curr_model, question=query, answer=answer, source=', '.join(list(set(src))))
    update_chat(model=curr_model, records = fetch_history(conn=conn, model=curr_model))
    

def execute_model(conn:Connection,query:str, model: str, tab:DeltaGenerator):
    vectorDB = search_from_vectordb(st.session_state['chatvectordb'])
    print(f" ***The selected Vector DB is*** {vectorDB}")
    retriever=vectorDB.as_retriever()
    with tab:
        with st.container():
            update_chat_window(conn=conn, query=query, curr_model=model, llm=get_model(model), retriever= retriever)

def process_query(conn:Connection,query:str, models: List[str], tablist:List[DeltaGenerator]):
