...
def fetch_history(conn:Connection, model:str) -> list[Any]:
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM chathistory WHERE model=? ORDER BY eventtime DESC", (model,))
    return cursor.fetchall()

def insert_chathistory(conn:Connection, model:str, question:str, answer:str, source:str) -> None:
    print("Inserting")
    ...
    cur.execute("BEGIN")
    try:
        cur.execute(command, (hash(question), model, question, answer, source, datetime.now(tz).strftime("%d-%m-%Y %H:%M:%S") ))
        conn.commit()
...
def insert(conn:Connection, id:int, filehash:str, filename:str) -> None:
    ...
    cur.execute("BEGIN")
    try:
        cur.execute(command, (id, filehash, filename, ))
        conn.commit()
    except conn.Error as e:
        print("Got an error: ", e)
        print("Aborting...")
