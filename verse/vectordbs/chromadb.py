...
def upload(documents: list[Document], embeddings: Embeddings) -> Chroma:
    vectordb = Chroma.from_documents(
        documents=documents, embedding=embeddings, persist_directory=db_persistent_path
    )
    vectordb.persist()
    vectordb = None
...
def create(embeddings: Embeddings) -> VectorStore:
    return Chroma(persist_directory=db_persistent_path, embedding_function=embeddings)
